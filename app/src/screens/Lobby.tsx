...
export function Lobby() {
  ...
  if (!room) {
    return (
      <div className="lobby rise">
        ...
        {joinError && <div style={{ color: 'var(--red-deep)', fontWeight: 800, marginTop: 12 }}>{t(joinError)}</div>}
        <button
    ...
    return (
      <div className="lobby rise">
        <div className="code-card">
          ...
          <h2 style={{ marginTop: 6 }}>{t('Ищем соперников')}<span className="dots-anim" /></h2>
          <div style={{ color: 'var(--ink-soft)', fontWeight: 800, marginTop: 6 }}>
            {t('Подбираем игроков. Партия начнётся сама через пару секунд.')}
          </div>
        </div>
        ...
        <button className="btn ghost block" style={{ maxWidth: 420, marginTop: 18 }} onClick={leaveGame}>{t('Отмена')}</button>
      </div>
    )
  }
  ...
  return (
    <div className="lobby rise">
      <div className="page-head" style={{ width: '100%', maxWidth: 420 }}>
        <button className="round-btn" onClick={leaveGame} aria-label={t('Назад')}>‹</button>
        <h1>{t('Комната')}</h1>
      </div>

      <div className="code-card">
        <div style={{ fontWeight: 800 }}>{t('Код комнаты')}</div>
        <div className="code-big">{room.room.code}</div>
        <div style={{ color: 'var(--ink-soft)', fontWeight: 800, fontSize: 14 }}>
          {t('Поделись кодом с друзьями. Пустые места займут боты.')}
        </div>
        <button
          className="btn accent block"
          style={{ marginTop: 14 }}
          onClick={() => { haptic('tap'); shareLink(link, `${t('Заходи в «')}${t(APP_NAME)}${t('»! Код комнаты: ')}${room.room.code}`) }}
        >
          {t('Пригласить друзей 📨')}
        </button>
      </div>
      ...
      {isHost ? (
        ...
        <div style={{ color: 'var(--ink-soft)', fontWeight: 800, marginTop: 18, textAlign: 'center' }}>
          {t('Ждём, пока хост начнёт игру')}<span className="dots-anim" />
        </div>
      )}
    </div>
  )
