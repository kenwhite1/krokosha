      ...
      {profile && (
        <div className="code-card" style={{ marginBottom: 18 }}>
          ...
          <div className="stat-strip" style={{ marginTop: 12 }}>
            ...
            <div className="stat-pill"><div className="v">{profile.played}</div><div className="l">Партий</div></div>
            <div className="stat-pill"><div className="v">{profile.bestStreak}</div><div className="l">Рекорд</div></div>
          </div>
        </div>
      )}
      {rows.length === 0 ? (
        <div className="empty-note">Пока пусто. Сыграй партию, чтобы попасть в таблицу!</div>
      ) : (
        <div className="board">
          {rows.map((r, i) => (
