      ...
      <div className="app">
        <div className="home" style={{ justifyContent: 'center' }}>
          <div className="brand" style={{ animation: 'pop-in .5s ease both' }}>
            ...
            <div className="brand-tag">{t('Загадываем слово')}<span className="dots-anim" /></div>
          </div>
        </div>
      </div>
    )
              ...
              {DIFFS.map(({ d, t: dt, s, emoji }) => (
                ...
                >
                  ...
                  <span className="tile-text">
                    <span className="tile-title">{t(dt)}</span>
  ...
  return (
    <div className="scrim center">
      ...
      <div className="sheet pop result">
        ...
        <h1>{won ? t('Ты победил!') : `${t('Победил ')}${t(result.winnerName)}`}</h1>
        <div className="result-sub">{won ? `${t('Твой счёт: ')}${result.youScore}` : t('В следующий раз повезёт!')}</div>

        <div className="standings">
          {result.standings.map((p, i) => (
            <div className={`standing ${i === 0 ? 'top' : ''}`} key={p.id}>
              <span className="rk">{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}</span>
              <span className="nm">{p.you ? t('Ты') : t(p.name)}</span>
        ...
        {won && (
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
            <span className="coin-chip">🪙 +{25 + Math.round(result.youScore / 4)}{t(' монет')}</span>
          </div>
        )}
